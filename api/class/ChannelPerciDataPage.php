...
//            echo "<post>";
    ...
    public function getPreciTypeByDate($sdate, $edate) {
        $sqlQuery = "select 
                    ...
                    order by DATE_FORMAT(valuedate, '%Y-%m-%d %H:%i') asc";
        ...
        if ($itemCount > 0) {
            ...
            usort($finalArr, function ($item1, $item2) {
                return strtotime($item1['valuedate']) <=> strtotime($item2['valuedate']);
            });
...
//    }
    ...
    function getFinalperciRecordData($tempArr, $event, $eventColVal) {
        ...
        for ($t = 0; $t < $uacnt - 1; $t++) {
            ...
            if (($tempvalue[$event] == 0 && $tempvalNext == 0) || ($tempvalue[$event] == !0 && $tempvalNext == 0)) {
                ...
                if (($tempvalue[$event] != 0 && $tempvalNext != 0 ) ||
                    ...
                    $finalArr[] = array(
                        'event' => $eventColVal,
                        'fullstartdate' => $lasttmptime,
                        'fullenddate' => $lasttmptimeNext,
                        'valuedate' => $tempvalue['valuedate'],
                        'valuetimestart' => $tempvalue['valuetime'],
                        'valuetimeend' => $tempvalueNext['valuetime']
                    );
                }
            }
        }
        ...
        if ($step + 1 == $uacnt && $tempvalNext != 0) {
            ...
            //Remove last element from final array to avoid duplicates
            array_pop($finalArr);
...
//            echo "<post>";
    ...
    public function findPrecipitationThisWeek() {
        $sqlQuery = "select (DATE_FORMAT(channelv.valuedate, '%v'))  perciweek , 
                       ...
                       where channelv.channelid ='0101' and 
                        DATE_FORMAT((channelv.datatime),'%v') = (select (DATE_FORMAT(max(datatime),'%v')) 
                        from channel_0101_values where  channelid ='0101')
                         and YEAR(channelv.datatime) =  (select YEAR(max(datatime))                        
    ...
    public function findPrecipitationThisMonth() {
        $sqlQuery = "select  CAST(sum(channelv.value) AS DECIMAL(10,2))  perciThisMonth 
                     from channel_0101_values channelv
                       where channelv.channelid ='0101' and  
                       DATE_FORMAT((channelv.datatime),'%m-%Y') = (select (DATE_FORMAT(max(datatime),'%m-%Y')) 
                       from channel_0101_values where  channelid ='0101')";
...
//                        echo "<post>";
    ...
    public function findMaxPrecipitationByDay($sdate) {
        ...
        $sqlQuery1Hr = " select fromdate , todate, maxValues max1Hrval from 
                        (SELECT CAST(t1.datatime - INTERVAL 60 MINUTE AS CHAR(50)) fromdate,   CAST(t1.datatime AS CHAR(50)) todate,
                            (SELECT CAST(sum(t2.value) AS DECIMAL(10,2) ) FROM channel_0103_values t2
                             WHERE t2.datatime
                                 BETWEEN (t1.datatime - INTERVAL 60 MINUTE) AND t1.datatime) AS maxValues
                        FROM channel_0103_values t1
                        WHERE 
                            t1.datatime >= (SELECT MIN(datatime) + INTERVAL 60 MINUTE FROM channel_0103_values)
                            and date(datatime) =:datevalue
                        ORDER BY t1.datatime) t
                        order by maxValues desc limit 1 ";
